...
function Client(config) {
  ...
  this._openSocket = function () {
    ...
    this._socket.on(comm.Message.EVENT_RANDOM_MATCH_START, function(params){
      self.handleMessage(comm.Message.EVENT_RANDOM_MATCH_START, params);
    });
  ...
  this.handleMessage = function (msg, params) {
    ...
    else if (msg == comm.Message.EVENT_RANDOM_MATCH_START) {
      this.handleEventRandomMatchStart(params);
    }
   ...
   */
  this.handleEventRandomMatchStart = function (params) {
    console.log('Random match started');
