 ...
 */
RuleBgCasual.prototype.getMoveActions = function(game, position, type, steps) {
  var actionList = [];
  
  // First, check status of the game: if game was started, if it is player's turn, etc.
  var result = Rule.prototype.validateMove.call(this, game, position, type, steps);
  if (!result) {
    actionList = [];
    return actionList;
  }
   ...
   */
  function addAction(moveActionType, pieceType, from, to) {
  ...
  try {
    ...
    if (this.havePiecesOnBar(game, type)) {
      /*
        ...
        Input data: position=null, steps=3
        Cases:
      ...
      */
      var destination = (type == model.PieceType.WHITE) ? (24 - steps) : (steps - 1);

      var topPieceType = model.State.getTopPieceType(game.state, destination);

      // There are no pieces at this point or the top piece is owned by player,
      // so place piece from bar to opponent's home field
      if ((topPieceType == null) || (topPieceType === type)) {
        addAction(
          model.MoveActionType.RECOVER, type, destination
        );
      }
      // The top piece is opponent's and is only one (i.e. the point is not blocked)
      else if (model.State.countAtPos(game.state, destination, topPieceType) === 1) {
        addAction(
          model.MoveActionType.HIT, topPieceType, destination
        );
      ...
      */
      ...
      if (normDestination >= 0) {
        ...
        if ((topPieceType == null) || (topPieceType === type)) {
          addAction(
            model.MoveActionType.MOVE, type, position, destination
          );
        }
      ...
      // If steps are just enought to reach position -1, bear piece
        ...
        addAction(
          model.MoveActionType.BEAR, type, position
        );
      }
      // If steps move the piece beyond -1 postion, the player can bear the piece,
      // only if there are no other pieces at higher positions
      else {
        var normSource = this.normPos(position, type);
        if (this.countAtHigherPos(game.state, normSource + 1, type) <= 0) {
          addAction(
            model.MoveActionType.BEAR, type, position
          );
        }
      ...
      */
      ...
      if ((destination >= 0) && (destination <= 23)) {
        var normDest = this.normPos(destination, type);
 ...
 */
RuleBgCasual.prototype.applyMoveActions = function(game, actionList) {
  for (var i = 0; i < actionList.length; i++) {
    var action = actionList[i];

    if (action.type === model.MoveActionType.MOVE) {
      this.move(game.state, action.from, action.to, action.pieceType);
    }
    else if (action.type === model.MoveActionType.RECOVER) {
      this.recover(game.state, action.position, action.pieceType);
    }
    else if (action.type === model.MoveActionType.HIT) {
      this.hit(game.state, action.position, action.pieceType);
    }
    else if (action.type === model.MoveActionType.BEAR) {
      this.bear(game.state, action.position, action.pieceType);
    }
  }
}
