 ...
 */
RuleBgCasual.prototype.rollDice = function(game) {
 ...
 */
RuleBgCasual.prototype.validateMove = function(game, player, piece, steps) {
 ...
 */
RuleBgCasual.prototype.calculateMoveWeights = function(state, movesLeft, pieceType, rootPiece, stopAtMax) {
  var weight = {};
  weight['max'] = 0;
  ...
  function calculateBranchWeights(state, moves, id, branchSum, level, branchMoves) {
    ...
    for (var p = 0; p < state.pieces[pieceType].length; p++) {
      var piece = state.pieces[pieceType][p];
      ...
      if (model.State.havePiecesOnBar(state, pieceType)) {
        ...
        if (model.State.getBarTopPiece(state, pieceType).id != piece.id) {
          continue;
        }
        //console.log('Bar');
      }
        ...
        var pos = model.State.getPiecePos(state, piece);
        if (model.State.getTopPiece(state, pos).id != piece.id) {
          continue;
        }
        //console.log('Pos', pos);
      }
      ...
      var tempState = model.Utils.deepCopy(state);
 ...
 */
RuleBgCasual.prototype.isMoveActionRestricted = function(state, movesLeft, piece, steps) {
  // 1. Try out all possible moves (for all of player's pieces).
  // 2. Sum the move values for all resulting branches with possible moves.
  // 3. Check if the move request of the player can be used in a branch that allows
  //    all move values to be used.
